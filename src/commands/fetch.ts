...
type FetchOpts = {
  ...
  name?: string;              // output type/file name (without .ts)
...
function maskPII(value: unknown, opts?: { maskEmail?: boolean; maskPhone?: boolean }): unknown {
  ...
  if (typeof value === "string") {
    ...
    if (opts?.maskPhone && s.replace(/\D/g, "").length >= 10) {
      s = s.replace(/\d/g, (d, i, a) => (i < a.length - 4 ? "*" : d));
    }
    return s;
  }
  return value;
...
export async function fetchCommand(opts: FetchOpts) {
  ...
  const name = opts.name ?? deriveNameFromUrl(u);
  ...
  const dts = await jsonToTypes(name, sanitized);
  const header =
